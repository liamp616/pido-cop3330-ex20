...
public class App {
    ...
    public static void main(String[] args) {
        ...
        if(state.equals("Wisconsin")) {
            ...
            myApp.calculateWisconsin(amount, county);
        } else if(state.equals("Illinois")) {
            myApp.calculateIllinois(amount);
        } else {
            myApp.calculateTotal(amount);
        }
    }
    ...
    public void calculateIllinois(double amount) {
        double tax = 0;
        double total = 0;
    ...
    public void calculateWisconsin(double amount, String county) {
        double tax = 0;
        double total = 0;

        if (county.equals("Eau Claire")) {
            tax = (amount * 0.05) + (amount * 0.005);
            total = tax + amount;
            System.out.println("The tax is $" + String.format("%.2f", tax) + ".");
            System.out.println("The total is $" + String.format("%.2f", total) + ".");
        } else if (county.equals("Dunn")) {
            tax = (amount * 0.05) + (amount * 0.004);
            total = tax + amount;
            System.out.println("The tax is $" + String.format("%.2f", tax) + ".");
            System.out.println("The total is $" + String.format("%.2f", total) + ".");
        } else {
